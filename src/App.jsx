...
const FullscreenViewer = ({ photo, photos, onClose, onEdit, onDelete, onToggleFav, favorites }) => {
  ...
  // Secure alignment checks in case dynamic shifts make `current` empty
  if (!current) {
    return null;
  }
...
export default function RomanticGallery() {
  ...
  const [showFavoritesOnly, setShowFavoritesOnly] = useState(false); // Added dedicated state filter
  ...
  // Fixed filtering matching logic
      ...
      <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100vh", background: "#080808", fontFamily: "'EB Garamond', serif", position: "relative" }}>
        ...
        <div style={{
          ...
          {[
            ...
            { id: "favs", icon: <HeartIcon filled={showFavoritesOnly} />, label: "Loved" },
          ].map(({ id, icon, label }) => {
            // Determine dynamic active coloring logic
            const isActive = id === "favs" ? showFavoritesOnly : (!showFavoritesOnly && view === id);
            ...
            return (
              <button key={id} onClick={() => {
                ...
                color: isActive ? "rgba(255,220,180,0.85)" : "rgba(255,255,255,0.25)",
