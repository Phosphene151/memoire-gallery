...
const UploadOverlay = ({ onClose, onAdd, showToast }) => {
  ...
  const submit = async () => {
    ...
    try {
      // 1. Upload file directly to your existing 'Photos' Storage Bucket
      ...
      // 2. Get the clean public asset URL from your bucket
      ...
      // 4. Send structured package upward to database insertion handler
      await onAdd({
...
export default function RomanticGallery() {
  ...
  const toggleFav = async (id, isCurrentFav) => {
    try {
      ...
      showToast("Could not sync favorite");
    }
  };
  ...
  const handleSaveEdit = async (id, updatedFields) => {
    try {
      ...
      setPhotos(p => p.map(x => x.id === id ? { ...x, ...updatedFields } : x));
      setEditPhoto(null);
  ...
  const handleAdd = async (newPhotoPayload) => {
    try {
      const { data, error } = await supabase
        .from('memories')
      ...
      showToast("Could not log data row");
    }
  };
